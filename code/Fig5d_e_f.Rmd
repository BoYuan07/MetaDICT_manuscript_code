...
knn_pred = function(train, test, Y_train, Y_test){
  test_pred <- class::knn(train = t(train), test = t(test), cl = Y_train, k = 25, prob = TRUE)
  probs <- attr(test_pred, "prob")
  positive_class <- "Class1"
  positive_probs <- ifelse(test_pred == positive_class, probs, 1 - probs)
  roc_object <- roc(Y_test, positive_probs, direction = "<")
