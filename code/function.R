...
metadict = function(O_list,alpha,beta,gamma,dist,meta.list,r = NULL, normalization = "uq", max_iter=10000,neighbor=5,sigma=10){
  m = length(O_list)
  d = nrow(dist)
...
community_detection = function(D, K = 50, resolution=1, method = "Louvain", min_k = 5){
...
pcoa.plot.discrete = function(A,gl,main,colorset = "Set1"){
...
pcoa.plot.continuous = function(A,gl,main){
  dist_matrix = bcdist(t(A))
  mds.stuff = cmdscale(dist_matrix, eig=T, x.ret=T)
  mds.var.per = round(mds.stuff$eig/sum(mds.stuff$eig)*100,1)
  mds.values = mds.stuff$points
