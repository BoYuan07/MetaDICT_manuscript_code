...
init_dataset = function(m,n,count,dist,k,k1,k3=100,k4=30,ez=50,lib_var=T,p = c(1/2,1/2)){
  d = nrow(count)
  O_list = list()
  w_list = list()
  X_list = list()
...
data = init_dataset(m,n,count,dist,floor(0.05*d),floor(0.05*d),ez=ez,lib_var = T)
O_list = data[[1]]
...
Y_train = meta_list[[1]]$Y
Y_test = meta_list[[2]]$Y
Y_f_train = meta_list[[1]]$Y_f
Y_f_test = meta_list[[2]]$Y_f
...
#filename = paste0("./table/pred_sim2/","pred", "_simuiter", simu.iter, ".csv")
