...
for(i in 1:3){
  ...
  res = rbind(res,c(adjustedRandIndex(community_detection(O,K=10)$cluster,diff_set),"Unprocessed",ez,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(community_detection(t(O),K=50)$cluster,meta$Y),"Unprocessed",ez,"Sample"))
  ...
  res = rbind(res,c(ari[which.max(avg_silwidth)],"Single Dataset",ez,"Taxa"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_metadict,diff_set),"MetaDICT",ez,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_metadict_s,meta$Y),"MetaDICT",ez,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_combatseq,diff_set),"ComBatSeq",ez,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_combatseq_s,meta$Y),"ComBatSeq",ez,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_mmuphin,diff_set),"MMUPHin",ez,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_mmuphin_s,meta$Y),"MMUPHin",ez,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_conqur,diff_set),"ConQuR",ez,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_conqur_s,meta$Y),"ConQuR",ez,"Sample"))
                            
}



#filename = paste0("./table/joint_cluster_signal/","sim_", simu.iter, ".csv")
