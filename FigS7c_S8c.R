...
for(i in 1:3){
  ...
  res = rbind(res,c(adjustedRandIndex(community_detection(O,K=10)$cluster,diff_set),"Unprocessed",n,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(community_detection(t(O),K=50)$cluster,meta$Y),"Unprocessed",n,"Sample"))
  ...
  res = rbind(res,c(ari[which.max(avg_silwidth)],"Single Dataset",n,"Taxa"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_metadict,diff_set),"MetaDICT",n,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_metadict_s,meta$Y),"MetaDICT",n,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_combatseq,diff_set),"ComBatSeq",n,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_combatseq_s,meta$Y),"ComBatSeq",n,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_mmuphin,diff_set),"MMUPHin",n,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_mmuphin_s,meta$Y),"MMUPHin",n,"Sample"))
  ...
  res = rbind(res,c(adjustedRandIndex(c_conqur,diff_set),"ConQuR",n,"Taxa"))
  res = rbind(res,c(adjustedRandIndex(c_conqur_s,meta$Y),"ConQuR",n,"Sample"))
}

#filename = paste0("./table/joint_cluster_n/","sim_", simu.iter, ".csv")
